...
--	@Shift_Code = '1'
DECLARE
@current_time   DATETIME = SYSDATETIME() at time zone 'UTC' at time zone (SELECT site_timezone FROM dbo.CommonParameters where site_id = @Site);
...
WITH CTE
     AS (SELECT CONVERT(VARCHAR, BD.started_on_chunck, 20) AS started_on_chunck, 
                ...
                SUM(PD.ideal) OVER(ORDER BY PD.dxhdata_id) AS cumulative_ideal, 
                SUM(PD.target) OVER(PARTITION BY PD.dxhdata_id) AS summary_target, 
                SUM(PD.target) OVER(ORDER BY BD.started_on_chunck) AS cumulative_target,  
                SUM(PD.actual) OVER(PARTITION BY PD.dxhdata_id) AS summary_actual, 
                SUM(PD.actual) OVER(ORDER BY BD.started_on_chunck) AS cumulative_actual,
                ...
                SUM(PD.setup_scrap) OVER(ORDER BY PD.dxhdata_id) AS cumulative_setup_scrap, 
                SUM(PD.other_scrap) OVER(PARTITION BY PD.dxhdata_id) AS summary_other_scrap, 
                SUM(PD.other_scrap) OVER(ORDER BY PD.dxhdata_id) AS cumulative_other_scrap, 
                ...
                CASE
                    ...
                    THEN 'multiple'
                    ELSE (CASE WHEN PD.productiondata_id IS NULL OR DH.dxhdata_id IS NULL THEN (CASE WHEN 
					(SELECT COUNT(productiondata_id) from productiondata where start_time > BD.started_on_chunck
					AND OD.order_number = order_number) >= 1 THEN OD.product_code ELSE '' END) ELSE PD.product_code END)
         ...
         (
             SELECT
				SUM(CASE
				WHEN FORMAT(PD1.start_time, 'yyyy-MM-dd HH') = FORMAT(@current_time, 'yyyy-MM-dd HH') AND PD1.target > (PD1.actual - PD1.setup_scrap - PD1.other_scrap) THEN FLOOR(PD1.target)
     ...
     SELECT started_on_chunck, 
            ...
            product_code_order,
			CASE
			WHEN summary_setup_minutes < 0 THEN 0
			WHEN summary_setup_minutes > 60 THEN 60
     ...
     --result_final
     FROM CTE3 ORDER BY started_on_chunck asc, start_time desc;
END
