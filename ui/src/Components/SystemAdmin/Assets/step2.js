...
export class Step2 extends Component {
	...
	componentDidMount() {
		...
		return Promise.all([actions.getUOM(this.props.user.site),
	...
	createTag = (e) => {
		e.preventDefault();
		const { status, name, uom_code, description, rollover, data_type, max_change, sites } = this.state;
		...
		if (_.isEmpty(validation)) {
			Axios.put(url, {
				...
				aggregation: 'SUM',
				asset_id: _.find(sites, { asset_code: this.props.asset_code }).asset_id,
