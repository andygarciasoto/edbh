...
export function getParametersOfTable(tableName, siteId) {
    ...
    switch (tableName) {
        case 'Workcell':
            parametersObject.matchParameters = 's.workcell_name = t.workcell_name AND s.workcell_description = t.workcell_description';
            parametersObject.updateSentence = 't.[workcell_name] = s.[workcell_name], t.[workcell_description] = s.[workcell_description]';
            parametersObject.insertSentence = `([workcell_name], [workcell_description], [entered_by], [entered_on], [last_modified_by], [last_modified_on]) VALUES (s.[workcell_name], s.[workcell_description], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on])`;
            break;
        case 'Asset':
        ...
        case 'DTReason':
            ...
            parametersObject.insertSentence = `([dtreason_code], [dtreason_name], [dtreason_description], [dtreason_category], [level], [reason1], [reason2], [status], [entered_by], [entered_on], [last_modified_by], [last_modified_on], [asset_id], [type]) VALUES (s.[dtreason_code], s.[dtreason_name], s.[dtreason_description], s.[dtreason_category], s.[level], s.[reason1], s.[reason2], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on], s.[asset_id], s.[type])`;
            break;
        case 'Shift':
            parametersObject.extraColumns = ', a.asset_id';
        ...
        case 'TFDUsers':
            ...
            parametersObject.updateSentence = `t.[Badge] = s.[Badge], t.[Username] = s.[Username], t.[First_Name] = s.[First_Name], t.[Last_Name] = s.[Last_Name], t.[Role] = s.[Role]`;
