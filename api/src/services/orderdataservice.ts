...
export class OrderDataService {
    ...
    public async getOrderAssembly(req: Request, res: Response) {
        ...
        let orderData: any;
        try {
            assembly = await this.commonparametersrepository.getAssemblyUrl(params.asset_code);
            const url = assembly[0] !== undefined ? assembly[0].assembly_url : undefined;
            if (url === undefined || url === null || !url) {
                return res.status(400).json({ message: "Bad Request - Site doesn't have a correct assembly URL" });
            } else {
