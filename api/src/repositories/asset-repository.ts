...
export class AssetRepository {
    ...
    public async getAssetsWithoutTag(site_id: number): Promise<any> {
        return await this.sqlServerStore.ExecuteQuery(`WITH cte1
        AS (SELECT A.asset_id,A.asset_code,A.asset_name,A.asset_description,A.asset_level,A.site_code,A.parent_asset_code,
            ...
            WHERE  A2.asset_id = ${site_id} AND A.asset_level = 'Cell' AND (T.tag_id IS NULL OR T.status = 'Inactive')),
        cte2
        AS (SELECT cte1.*
            FROM   cte1
                   INNER JOIN dbo.tag T ON cte1.asset_id = T.asset_id AND T.status = 'Active')
        SELECT cte1.*
        FROM   cte1
          LEFT JOIN cte2 ON cte1.asset_id = cte2.asset_id WHERE cte2.asset_id IS NULL AND cte1.status = 'Active'`);
    }
}