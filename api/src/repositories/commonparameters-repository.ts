...
export class CommonParametersRepository {
    ...
    public async getCommonParametersBySite(site_id: number): Promise<any> {
        return await this.sqlServerStore.ExecuteQuery(`SELECT [Asset].[asset_code] AS site_code,[CommonParameters].[site_name],[CommonParameters].[production_day_offset_minutes],
        [CommonParameters].[site_timezone],[CommonParameters].[ui_timezone],[CommonParameters].[default_target_percent_of_ideal],[CommonParameters].[default_setup_minutes],
        [CommonParameters].[default_routed_cycle_time],[CommonParameters].[language],[CommonParameters].[status],
