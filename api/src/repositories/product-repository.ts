...
export class ProductRepository {
    ...
    public async insertProduct(part_number: string): Promise<any> {
        return await this.sqlServerStore.ExecuteQuery(`exec dbo.spLocal_EY_DxH_Put_Products_From_IoT '${part_number}', '${part_number}', '${part_number}', '', '', '', '', '', '', '', 'Active', 'SQL manual entry', '${moment.utc().format('YYYY-MM-DD HH:MM')}';`);
    }
}