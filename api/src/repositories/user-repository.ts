...
export class UserRepository {
    ...
    public async findUserBySite(site_id: number): Promise<any> {
        return await this.sqlServerStore.ExecuteQuery(`SELECT TFD.Badge, TFD.Username, TFD.First_Name, TFD.Last_Name, R.Name as Role, TFD.role_id, TFD.status,
        A.site_code FROM dbo.TFDUsers TFD
    ...
    public async putUser(badge: string, username: string, first_name: string, last_name: string, role_id: number, status: string, site_id: number): Promise<any> {
        return await this.sqlServerStore.ExecuteQuery(`exec spLocal_EY_DxH_Put_Users N'${badge}', N'${username}', N'${first_name}', N'${last_name}', ${role_id}, N'${status}', ${site_id}`);
    }
