            ...
            worksheet.eachRow((row, rowNumber) => {
              ...
              columns.forEach((col, colNumber) => {
                if (rowNumber === 1) {
                  ...
                  updateRow += getValuesFromHeaderTable(constants[worksheet.name], constants[worksheet.name][colNumber], row.getCell(colNumber + 1).value);
                }
              });
              if (rowNumber !== 1) {
...
router.get('/export_data', async function (req, res) {
  ...
  let promiseArray = [];
  promiseArray.push(constants.getPromise(constants.WorkcellSQL(site_id), 'Workcell'));
  ...
  promiseArray.push(constants.getPromise(constants.AssetDisplaySystemSQL(site_id), 'AssetDisplaySystem'));
