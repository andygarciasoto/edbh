...
function getParametersOfTable(tableName) {
  ...
  switch (tableName) {
    ...
    case 'CommonParameters':
      ...
      parametersObject.updateSentence = `t.[site_name] = s.[site_name], t.[production_day_offset_minutes] = s.[production_day_offset_minutes], t.[site_timezone] = s.[site_timezone], t.[ui_timezone] = s.[ui_timezone], t.[escalation_level1_minutes] = s.[escalation_level1_minutes], t.[escalation_level2_minutes] = s.[escalation_level2_minutes], t.[default_target_percent_of_ideal] = s.[default_target_percent_of_ideal], t.[default_setup_minutes] = s.[default_setup_minutes], t.[default_routed_cycle_time] = s.[default_routed_cycle_time], t.[setup_lookback_minutes] = s.[setup_lookback_minutes], t.[inactive_timeout_minutes] = s.[inactive_timeout_minutes], t.[language] = s.[language], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], t.[last_modified_by] = s.[last_modified_by], t.[last_modified_on] = s.[last_modified_on], t.[summary_timeout] = s.[summary_timeout]`;
      parametersObject.insertSentence = `([site_id], [site_name], [production_day_offset_minutes], [site_timezone], [ui_timezone], [escalation_level1_minutes], [escalation_level2_minutes], [default_target_percent_of_ideal], [default_setup_minutes], [default_routed_cycle_time], [setup_lookback_minutes], [inactive_timeout_minutes], [language], [status], [entered_by], [entered_on], [last_modified_by], [last_modified_on], [summary_timeout]) VALUES (s.[site_id], s.[site_name], s.[production_day_offset_minutes], s.[site_timezone], s.[ui_timezone], s.[escalation_level1_minutes], s.[escalation_level2_minutes], s.[default_target_percent_of_ideal], s.[default_setup_minutes], s.[default_routed_cycle_time], s.[setup_lookback_minutes], s.[inactive_timeout_minutes], s.[language], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on], s.[summary_timeout])`;
      break;
    case 'UOM':
      parametersObject.extraColumns = ', a.asset_id as site_id';
      ...
      parametersObject.updateSentence = `t.[UOM_name] = s.[UOM_name], t.[UOM_description] = s.[UOM_description], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], t.[last_modified_by] = s.[last_modified_by], t.[last_modified_on] = s.[last_modified_on], t.[decimals] = s.[decimals]`;
