...
function getParametersOfTable(tableName) {
  ...
  switch (tableName) {
    case 'Asset':
      ...
      parametersObject.updateSentence = `t.[asset_name] = s.[asset_name], t.[asset_description] = s.[asset_description], t.[asset_level] = s.[asset_level], t.[value_stream] = s.[value_stream], 
      t.[automation_level] = s.[automation_level], t.[include_in_escalation] = s.[include_in_escalation], t.[grouping1] = s.[grouping1], t.[grouping2] = s.[grouping2], t.[grouping3] = s.[grouping3], 
      t.[grouping4] = s.[grouping4], t.[grouping5] = s.[grouping5], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], t.[last_modified_by] = s.[last_modified_by], 
      t.[last_modified_on] = s.[last_modified_on]`;
      parametersObject.insertSentence = `([asset_code], [asset_name], [asset_description], [asset_level], [site_code], [parent_asset_code], [value_stream], [automation_level], [include_in_escalation], 
        [grouping1], [grouping2], [grouping3], [grouping4], [grouping5], [status], [entered_by], [entered_on], [last_modified_by], [last_modified_on])
        VALUES 
        (s.[asset_code], s.[asset_name], s.[asset_description], s.[asset_level], s.[site_code], s.[parent_asset_code], s.[value_stream], s.[automation_level], s.[include_in_escalation], 
          s.[grouping1], s.[grouping2], s.[grouping3], s.[grouping4], s.[grouping5], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on])`;
      break;
    case 'DTReason':
      ...
      parametersObject.updateSentence = `t.[dtreason_name] = s.[dtreason_name], t.[dtreason_description] = s.[dtreason_description], t.[dtreason_category] = s[dtreason_category], t.[reason1] = s.[reason1], 
      t.[reason2] = s.[reason2], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], t.[last_modified_by] = s.[last_modified_by], t.[last_modified_on] = s.[last_modified_on]`;
      parametersObject.insertSentence = `([dtreason_code], [dtreason_name], [dtreason_description], [dtreason_category], [reason1], [reason2], [status], [entered_by], [entered_on], [last_modified_by], 
      [last_modified_on], [asset_id]) 
      VALUES 
      (s.[dtreason_code], s.[dtreason_name], s.[dtreason_description], s.[dtreason_category], s.[reason1], s.[reason2], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on], 
      s.[asset_id])`;
      break;
    case 'Shift':
      parametersObject.extraColumns = ', a.asset_id';
      ...
      parametersObject.updateSentence = `t.[shift_name] = s.[shift_name], t.[shift_description] = s.[shift_description], t.[shift_sequence] = s.[shift_sequence], t.[start_time] = s.[start_time], 
      t.[end_time] = s.[end_time], t.[duration_in_minutes] = s.[duration_in_minutes], t.[valid_from] = s.[valid_from], t.[valid_to] = s.[valid_to], t.[team_code] = s.[team_code], 
      t.[is_first_shift_of_day] = s.[is_first_shift_of_day], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], t.[last_modified_by] = s.[last_modified_by], 
      t.[last_modified_on] = s.[last_modified_on]`;
      parametersObject.insertSentence = `([shift_code], [shift_name], [shift_description], [shift_sequence], [start_time], [end_time], [duration_in_minutes], [valid_from], [valid_to], [team_code], 
      [is_first_shift_of_day], [status], [entered_by], [entered_on], [last_modified_by], [last_modified_on], [asset_id]) 
      VALUES 
      (s.[shift_code], s.[shift_name], s.[shift_description], s.[shift_sequence], s.[start_time], s.[end_time], s.[duration_in_minutes], s.[valid_from], s.[valid_to], s.[team_code], 
        s.[is_first_shift_of_day], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on], s.[asset_id])`;
      break;
    case 'Tag':
      parametersObject.extraColumns = ', a.asset_id';
      parametersObject.joinSentence = `JOIN dbo.Asset a ON s.asset_code = a.asset_code`;
    ...
    case 'CommonParameters':
      ...
      parametersObject.updateSentence = `t.[site_name] = s.[site_name], t.[production_day_offset_minutes] = s.[production_day_offset_minutes], t.[site_timezone] = s.[site_timezone], 
    ...
    case 'UOM':
      ...
      parametersObject.updateSentence = `t.[UOM_name] = s.[UOM_name], t.[UOM_description] = s.[UOM_description], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], 
      t.[last_modified_by] = s.[last_modified_by], t.[last_modified_on] = s.[last_modified_on], t.[decimals] = s.[decimals]`;
      parametersObject.insertSentence = `([UOM_code], [UOM_name], [UOM_description], [status], [entered_by], [entered_on], [last_modified_by], [last_modified_on], [site_id], [decimals]) 
      VALUES 
      (s.[UOM_code], s.[UOM_name], s.[UOM_description], s.[status], s.[entered_by], s.[entered_on], s.[last_modified_by], s.[last_modified_on], s.[site_id], s.[decimals])`;
      break;
    case 'Unavailable':
      parametersObject.extraColumns = ', a.asset_id';
      parametersObject.joinSentence = `JOIN dbo.Asset a ON s.site_code = a.asset_code`;
      parametersObject.matchParameters = 's.unavailable_code = t.unavailable_code AND s.asset_id = t.asset_id';//consultar con Andres
      parametersObject.updateSentence = `t.[unavailable_name] = s.[unavailable_name], t.[unavailable_description] = s.[unavailable_description], t.[start_time] = s.[start_time], t.[end_time] = s.[end_time], 
      t.[duration_in_minutes] = s.[duration_in_minutes], t.[valid_from] = s.[valid_from], t.[valid_to] = s.[valid_to], t.[status] = s.[status], t.[entered_by] = s.[entered_by], t.[entered_on] = s.[entered_on], 
      t.[last_modified_by] = s.[last_modified_by], t.[last_modified_on] = s.[last_modified_on]`;
      parametersObject.insertSentence = `([unavailable_code], [unavailable_name], [unavailable_description], [start_time], [end_time], [duration_in_minutes], [valid_from], [valid_to], [status], [entered_by], 
      [entered_on], [last_modified_by], [last_modified_on], [asset_id]) 
      VALUES 
      (s.[unavailable_code], s.[unavailable_name], s.[unavailable_description], s.[start_time], s.[end_time], s.[duration_in_minutes], s.[valid_from], s.[valid_to], s.[status], s.[entered_by], 
    ...
    case 'TFDUsers':
      ...
      parametersObject.updateSentence = `t.[Username] = s.[Username], t.[First_Name] = s.[First_Name], t.[Last_Name] = s.[Last_Name], t.[Role] = s[Role]`;
      parametersObject.insertSentence = `([Badge], [Username], [First_Name], [Last_Name], [Role], [Site]) 
      VALUES 
      (s.[Badge], s.[Username], s.[First_Name], s.[Last_Name], s.[Role], s.[Site])`;
      break;
  }
  ...
  workbook.xlsx.readFile(file.path)
    .then(() => {
      workbook.eachSheet((worksheet, sheetId) => {
        ...
        mergeQuery += tableSourcesValues.join(',') + `) AS S(${columns.map(e => e.header)}) ${parameters.joinSentence}) as s 
        ON (${parameters.matchParameters}) 
        WHEN MATCHED 
        THEN UPDATE SET 
        ${parameters.updateSentence} 
